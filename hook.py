...
def process(inputs, ctx, **kwargs):
    image = list(inputs.values())[0]

    if len(image.shape) == 0:
        bts = io.BytesIO(image)
        image = PilImage.open(bts)
    elif len(image.shape) == 1:
        bts = io.BytesIO(image[0])
        image = PilImage.open(bts)
    ...
    filtered = filter.filter(image, image, render_factor=render_factor)

    image_bytes = io.BytesIO()
    filtered.save(image_bytes, 'JPEG')
